# Timo's Blog: http://timoponce.blogspot.com/2012/04/binary-tree-implementation-on-ruby.html
...
class BTree
	...
	def initialize(value = nil)
		#creating a new node with nil value, left, right and parent node
		@root_node = Node.new(value) 
	end

	# every time we adding a new integer add is looping throug it
	def add( *args) # it can take * any number of arguments
		args.each do |arg|
			add_single arg # add single
		end
	end

	# checking if root_node is nil, if not then comparing
